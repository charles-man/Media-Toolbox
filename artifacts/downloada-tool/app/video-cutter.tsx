...
export default function VideoCutterScreen() {
  ...
  const handleCut = async () => {
    ...
    setResultMessage(`${t.cutVideo}: ${startTime} → ${endTime} ✓`);
    setStatus("success");
  ...
  const handleReset = () => {
    ...
    setStartTime("00:00"); setEndTime("00:30");
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.title, { color: colors.foreground }]}>{t.cutVideo}</Text>
      <Text style={[styles.description, { color: colors.mutedForeground }]}>
        Trim any video by setting a start and end time. Extract only the segment you need.
      </Text>

      {/* URL input */}
      <Text style={[styles.sectionLabel, { color: colors.mutedForeground }]}>{t.videoUrl}</Text>
      {!pickedFileUri && (
      ...
      {/* Trim range — fixed height */}
      <Text style={[styles.sectionLabel, { color: colors.mutedForeground }]}>{t.trimRange}</Text>
      ...
      <View style={[styles.durationBar, { backgroundColor: colors.muted }]}>
        ...
        <Text style={[styles.durationText, { color: colors.mutedForeground }]}>
          {t.segment}: {startTime} → {endTime}
        </Text>
      </View>

      {status === "idle" && (
        <TouchableOpacity style={[styles.actionButton, { backgroundColor: accentColor }]} onPress={handleCut} activeOpacity={0.85}>
          <Feather name="scissors" size={18} color="#fff" style={{ marginRight: 8 }} />
          <Text style={styles.actionLabel}>{t.cutVideo}</Text>
        </TouchableOpacity>
      )}
      ...
      {(status === "success" || status === "error") && (
        <View style={[styles.resultCard, { backgroundColor: status === "success" ? colors.success + "12" : colors.destructive + "12", borderColor: status === "success" ? colors.success : colors.destructive }]}>
          ...
          <TouchableOpacity style={[styles.resetButton, { borderColor: colors.border }]} onPress={handleReset}>
            <Text style={[styles.resetLabel, { color: colors.foreground }]}>{t.cutAnother}</Text>
          </TouchableOpacity>
        </View>
      )}
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  urlInput: { flex: 1, fontSize: 15, paddingVertical: 0 },
  ...
  uploadDividerRow: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 10 },
  ...
  trimRow: { flexDirection: "row", alignItems: "center", marginBottom: 12, gap: 8 },
  timeBox: { flex: 1, height: 96, borderRadius: 14, borderWidth: 1, alignItems: "center", justifyContent: "center", paddingVertical: 12 },
  timeBoxLabel: { fontSize: 11, fontWeight: "600", letterSpacing: 0.5, marginBottom: 6 },
  timeInput: { fontSize: 24, fontWeight: "700", width: "100%", textAlign: "center", paddingVertical: 0, lineHeight: 30, height: 30 },
  timeFormat: { fontSize: 10, marginTop: 6, letterSpacing: 0.3 },
  arrowContainer: { width: 36, alignItems: "center", justifyContent: "center", gap: 4 },
  arrowLine: { width: 1, height: 16, borderRadius: 1 },
  arrowCircle: { width: 32, height: 32, borderRadius: 16, alignItems: "center", justifyContent: "center" },
  durationBar: { flexDirection: "row", alignItems: "center", gap: 6, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 10, marginBottom: 24 },
